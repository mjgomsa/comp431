HEADER = "HELO "
FOOTER = " pleased to meet you"

# HELO <whitespace> <domain> <nullspace> <CRLF>
def parseHELO(code):
    if len(code) <= len(HEADER):
        print("Wrong length")
        return False
    ...
    if not code.startswith(HEADER):
        print("Missing HEADER at start")
        return False
    if parseCRLF(code[-1]) is False:
        print("Doesn't end with CRLF")
        return False
    ...
    if '\n' in code:
        print("Has more than one instance of CRLF")
        return False

    ind_startDomain = findWhitespaceIndex(code, False) - 1
    ind_endDomain = findNullspaceIndex(code, True)
    if (ind_endDomain < -1):
        ind_endDomain = ind_endDomain + 1 
    else:
        ind_endDomain = len(code)
    payload_domain = code[ind_startDomain:ind_endDomain]
    # print("~~" + payload_domain + "~~")
    return parseDomain(payload_domain) == 0
...
def findNullspaceIndex(stringIn, parseBackwards):
    walkingString = stringIn
    ...
    if parseBackwards:
        walkingString = stringIn[::-1]
    
    while parseNullspace1(walkingString[:returnIndex])==0:
        returnIndex += 1
...
def checkSuffix(inStr):
    if not inStr.startswith(FOOTER):
        return False
    ending = inStr[len(FOOTER):]

    if not parseCRLF(ending[-1]):
        return False
    
    val = parseNullspace1(ending[:-1])
    return val == 0

def findWhitespaceIndex(stringIn, parseBackwards):
    walkingString = stringIn

    if len(stringIn) == 0:
        print("findWhitespaceIdx empty")
        return -1
    returnIndex = 1
    ...
    while parseNewWhitespace(walkingString[:returnIndex]):
        returnIndex += 1
...
def HELObool(code):
    if len(code) <= len(HEADER) + len(FOOTER):
        print("Wrong length")
        return False
    if HEADER not in code or FOOTER not in code:
        print("Wrong format")
        return False
    if not code.startswith(HEADER):
        print("Missing HEADER at start")
    ...
    suffix_split = code.index(FOOTER)
    suffix = code[suffix_split:]

    if not checkSuffix(suffix):
        print("checksuffix failed")
        return False

    payload = code[len(HEADER) : suffix_split-1]
    headWhitespaceIndx = findWhitespaceIndex(payload, False)
    tailWhitespaceIndx = findWhitespaceIndex(payload, True)

    domainStr = payload[headWhitespaceIndx:tailWhitespaceIndx]

    return parseDomain(domainStr) == 0
...
def parseNewWhitespace(string):
    if len(string) == 0:    
        return False
        
    for i in range(len(string)):
        val = parseSpace(string[i])
        if val != 0:
            return False
